...
namespace OpenTK.Wpf
{
	...
	internal sealed class GLWpfControlRenderer : IDisposable
	{
		...
		public GLWpfControlRenderer(GLWpfControlSettings settings, IntPtr dxhwnd)
		{
			_context = new DxGlContext(settings, dxhwnd);
			// Placeholder transforms.
			TranslateTransform = new TranslateTransform(0, 0);
			FlipYTransform = new ScaleTransform(1, 1);
		}

		public void ReallocateFramebufferIfNeeded(double width, double height)
		{
			...
			if (_context.dComp.GetD3D11Texture2D() == null || FramebufferWidth != newWidth || FramebufferHeight != newHeight)
			{
